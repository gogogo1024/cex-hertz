...
+-------------------+      +-------------------+      +-------------------+
                          ...
                          +-------------------+   +-------------------+   
```
- 网关根据 symbol 路由到对应 Worker 节点，Worker 节点内部撮合引擎处理业务。
- 分区表和节点信息由后端动态维护，支持扩容、迁移、故障恢复。
...
- **任务状态管理与监控**
  ...
  - 迁移过程中自动收集关键指标（如迁移耗时、消息延迟、数据一致性），便于运维和优化。
...
- **与网关协同**
  - 迁移任务完成后，自动通知 APISIX 网关同步最新分区表和路由规则，实现流量无缝切换。

### 6.3 撮合引擎实例
- Worker 节点内部运行撮合引擎实例，负责分配到本节点的 symbol 的撮合服务。
- 支持多线程/协程并发撮合，订单簿、行情等业务模块解耦。
