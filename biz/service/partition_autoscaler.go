...
func (a *PartitionAutoScaler) scaleIfNeeded() {
	...
	partitionLoad := make(map[string]int) // 分区ID -> QPS
	symbolLoad := make(map[string]int)    // symbol -> QPS
	...
	var needUpdate bool
	newPT := pt.DeepCopy() // 使用深拷贝，保证分区表变更安全
	...
	for pid, p := range newPT.Partitions {
		...
		for _, symbol := range p.Symbols {
			if symbolLoad[symbol] > 800 { // symbol 级别热点阈值
		...
		if len(hotSymbols) > 0 {
			...
			// 为每个热点 symbol 新建分区，并分配到空闲 worker
			for _, s := range hotSymbols {
				idleWorker := a.selectIdleWorker(wlp)
				newPID := pid + "_hot_" + s + "_" + time.Now().Format("150405")
				...
				// ---同步更新SymbolToPartition---
				// 先移除 symbol 在原分区的映射
				oldList := newPT.SymbolToPartition[s]
				var newList []string
				...
				newPT.SymbolToPartition[s] = newList
			}
			needUpdate = true
		}
	}
